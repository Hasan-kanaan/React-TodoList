...
export default function Team() {
  const { allUsers, teamTasksPlusDue, fetchTasks} = useOutletContext();
  const [tasks, setTasks] = useState(teamTasksPlusDue);
    ...
    {tasks.length > 0 ? (
      tasks.map((task) => (
        ...
        <div className="empty-task">
          <p>You have zero team tasks in progress! Your team loves you!</p>
        </div>
      )}
    </div>
  </div>
